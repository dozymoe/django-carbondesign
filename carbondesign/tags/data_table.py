...
class DataTable(Node):
    ...
    "Available variants."
    NODE_PROPS = ('style', 'pager', 'pager_sizes', 'sortable')
...
"""
    ...
    def render_tmpl_header(self, values, context):
        if 'title' not in self.slots and 'description' not in self.slots:
            return ''
...
"""
    ...
    def render_tmpl_toolbar(self, values, context):
        if 'batch_actions' not in self.slots and\
                'toolbar_overflow' not in self.slots and\
                'toolbar' not in self.slots:
            return ''
...
class TableToolbarSearch(Node):
    ...
    def render_default(self, values, context):
        template = """
<div class="bx--toolbar-search-container-persistent {class}" {props}>
  <div data-search class="bx--search bx--search--sm" role="search">
