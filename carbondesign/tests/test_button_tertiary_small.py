...
class ButtonTertiarySmallTest(SimpleTestCase):
    maxDiff = None

    def test_default(self):
        template = """
{% load carbondesign %}
{% Button small=True variant="tertiary" type="button" %}
  Button
{% endButton %}
"""
        expected = """
<button class="bx--btn bx--btn--tertiary bx--btn--sm" type="button">
  Button
</button>
"""
    ...
    def test_disabled(self):
        template = """
{% load carbondesign %}
{% Button small=True variant="tertiary" type="button" disabled=True %}
  Button
{% endButton %}
"""
        expected = """
<button class="bx--btn bx--btn--tertiary bx--btn--sm" disabled="disabled" type="button">
  Button
</button>
"""
        rendered = compare_template(template, expected)
...
"""
    ...
    def test_icon_only(self):
        template = """
{% load carbondesign %}
{% IconButton variant="tertiary" small=True %}
  {% Slot 'icon' %}
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
        viewBox="0 0 32 32">
      <path d="M17 15L17 8 15 8 15 15 8 15 8 17 15 17 15 24 17 24 17 17 24 17 24 15z"></path>
...
"""
        expected = """
<button class="bx--btn bx--btn--tertiary bx--btn--sm bx--btn--icon-only bx--tooltip__trigger bx--tooltip--a11y bx--tooltip--bottom bx--tooltip--align-center">
