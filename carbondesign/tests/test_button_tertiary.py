...
class ButtonTertiaryTest(SimpleTestCase):
    maxDiff = None
...
"""
    ...
    def test_disabled(self):
        template = """
{% load carbondesign %}
{% Button variant="tertiary" type="button" disabled=True %}
  Button
{% endButton %}
"""
        expected = """
<button class="bx--btn bx--btn--tertiary" type="button" disabled="disabled">
  Button
</button>
"""
        rendered = compare_template(template, expected)
...
"""
        expected = """
<button class="bx--btn bx--btn--tertiary bx--btn--icon-only bx--tooltip__trigger bx--tooltip--a11y bx--tooltip--bottom bx--tooltip--align-center">
